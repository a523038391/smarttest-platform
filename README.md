...
后端版本管理默认关闭。启用时必须把唯一受管仓库配置为绝对目录：

    $env:VERSION_CONTROL_ENABLED="true"
    $env:VERSION_CONTROL_ROOT="D:\automation-projects\managed-repository"

`/api/v1/version-control/*` 仅允许管理员访问；`AUTH_REQUIRED=false` 时仅为本地开发和测试兼容而放行。接口不接受命令、仓库路径、远程名或分支参数，只操作配置目录和固定的 `origin`。拉取仅在有效仓库、已配置 `origin` 且工作区干净时执行 fast-forward-only；发布固定执行全量暂存、可选提交及 `git push -u origin HEAD`，没有新改动时仍可推送本地未发布提交。

Git 子进程禁用交互式凭据提示，使用固定超时、参数数组和进程内线程锁；API 不返回远程 URL、Git 原始输出或可能包含凭据的错误信息。发布会拒绝 `.env`（仅 `.env.example` 例外）、私钥/证书容器、数据库及备份类文件名，并在拒绝时撤销暂存。远程认证应由服务运行账户的受限凭据存储提供，不要把 Token 写入远程 URL 或仓库文件。Windows 上每次 Git 操作会安全读取当前启用的系统代理，拒绝含凭据或格式异常的代理地址，不会把代理地址返回给页面。
