...
Git 子进程禁用交互式凭据提示，使用固定超时、参数数组和进程内线程锁；API 不返回远程 URL、Git 原始输出或可能包含凭据的错误信息。发布会拒绝 `.env`（仅 `.env.example` 例外）、私钥/证书容器、数据库及备份类文件名，并在拒绝时撤销暂存。远程认证应由服务运行账户的受限凭据存储提供，不要把 Token 写入远程 URL 或仓库文件。
